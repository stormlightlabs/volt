...
/**
 * Check if value is a wrapped signal (from wrapSignal in evaluator)
 */
function isWrappedSignal(value: unknown): boolean {
  return (value !== null
    && typeof value === "object"
...
/**
 * Unwrap a value if it's a signal or wrapped signal
 */
function unwrapIfSignal(value: unknown): unknown {
  if (isWrappedSignal(value)) {
    ...
    case "object": {
      if (value !== null) {
        for (const [key, value_] of Object.entries(value)) {
          const unwrapped = unwrapIfSignal(value_);
          classes.set(key, Boolean(unwrapped));
        }
      }
      break;
    }
